...
def to_tensor(
    ...
    sdata : spatialdata.SpatialData
        `.table.uns['tensor']` : np.ndarray
            3D numpy array of shape (len(layers), sdata.table.n_obs, sdata.table.n_vars)
    """

    # Build tensor from specified layers
    tensor = []
    for l in layers:
        tensor.append(sdata.table.to_df().values)
    ...
    sdata.table.uns["tensor"] = np.array(tensor)