...
class TestGeometry(unittest.TestCase): 
    ...
    def test_sjoin_points(self):
        self.data = bt.geo.sjoin_points(sdata=self.data, points_key="transcripts", shape_keys=["cell_boundaries", "nucleus_boundaries"])
    ...
    def test_sjoin_shapes(self):
        self.data = bt.geo.sjoin_shapes(sdata=self.data, instance_key="cell_boundaries", shape_keys=["nucleus_boundaries"])
    ...
    def test_get_points(self):
        ...
        pd_sync = bt.geo.get_points(sdata=self.data, points_key="transcripts", astype="pandas", sync=True)
        pd_no_sync = bt.geo.get_points(sdata=self.data, points_key="transcripts", astype="pandas", sync=False)
        ...
        gdf_sync = bt.geo.get_points(sdata=self.data, points_key="transcripts", astype="geopandas", sync=True)
        gdf_no_sync = bt.geo.get_points(sdata=self.data, points_key="transcripts", astype="geopandas", sync=False)
        ...
        dd_sync = bt.geo.get_points(sdata=self.data, points_key="transcripts", astype="dask", sync=True)
        dd_no_sync = bt.geo.get_points(sdata=self.data, points_key="transcripts", astype="dask", sync=False)
    ...
    def test_get_shape(self):
        ...
        sync = bt.geo.get_shape(sdata=self.data, shape_key="nucleus_boundaries", sync=True)
        no_sync = bt.geo.get_shape(sdata=self.data, shape_key="nucleus_boundaries", sync=False)
    ...
    def test_set_points_metadata(self):
        ...
        dataframe_metadata = pd.DataFrame({"0": list_metadata, "1": list_metadata, "2": list_metadata})
        column_names = ["list_metadata", "series_metadata", "dataframe_metadata0", "dataframe_metadata1", "dataframe_metadata2"]

        bt.geo.set_points_metadata(sdata=self.data, points_key="transcripts", metadata=list_metadata, column_names=column_names[0])
        bt.geo.set_points_metadata(sdata=self.data, points_key="transcripts", metadata=series_metadata, column_names=column_names[1])
    ...
    def test_set_shape_metadata(self):
        ...
        bt.geo.set_shape_metadata(sdata=self.data, shape_key="cell_boundaries", metadata=list_metadata, column_names=column_names[0])
        bt.geo.set_shape_metadata(sdata=self.data, shape_key="cell_boundaries", metadata=series_metadata, column_names=column_names[1])
    ...
    def test_get_points_metadata(self):
        ...
        pd_metadata_single = bt.geo.get_points_metadata(sdata=self.data, points_key="transcripts", metadata_keys=column_names[0], astype="pandas")
        dd_metadata_single = bt.geo.get_points_metadata(sdata=self.data, points_key="transcripts", metadata_keys=column_names[0], astype="dask")
    ...
    def test_get_shape_metadata(self):
        ...
        bt.geo.set_shape_metadata(sdata=self.data, shape_key="cell_boundaries", metadata=list_metadata, column_names=column_names[0])
        bt.geo.set_shape_metadata(sdata=self.data, shape_key="cell_boundaries", metadata=series_metadata, column_names=column_names[1])
        metadata_single = bt.geo.get_shape_metadata(sdata=self.data, shape_key="cell_boundaries", metadata_keys=column_names[0])
