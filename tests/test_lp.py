...
class TestLp(unittest.TestCase): 
    ...
    def test_lp(self):
        lp_columns = ['cell_boundaries', 'feature_name', 'cell_edge', 'cytoplasmic', 'none', 'nuclear', 'nuclear_edge']
    ...
    def test_lp_diff_discrete(self):
        lp_diff_discrete_columns = ['feature_name', 'pattern', 'phenotype', 'dy/dx', 'std_err', 'z', 'pvalue', 'ci_low', 'ci_high', 'padj', '-log10p', '-log10padj', 'log2fc']
        ...
        bt.tl.lp_diff_discrete(sdata=self.data, instance_key="cell_boundaries", phenotype="cell_stage")
    ...
    def test_lp_diff_discrete_error(self):        
        error_test = []
        for i in range(len(self.data.shapes['cell_boundaries'])):
            if self.data.shapes['cell_boundaries']['cell_boundaries_area'][i] > self.data.shapes['cell_boundaries']['cell_boundaries_area'].median():
                error_test.append(1)
            else:
                error_test.append(0)
        ...
        with self.assertRaises(KeyError):
            bt.tl.lp_diff_discrete(sdata=self.data, instance_key="cell_boundaries", phenotype="error_test")
    ...
    def test_lp_diff_continuous(self):
        ...
        for column in lp_diff_continuous_columns:
            self.assertTrue(column in self.data.table.uns["diff_cell_boundaries_area"].columns)
